...
async def add_user(user: UserInDB, conn: asyncpg.Pool) -> User:
    return await conn.fetchrow("INSERT INTO users (username, hashed_password) VALUES ($1, $2) RETURNING username",
                               user.username, user.hashed_password)

async def get_user(username: str, conn: asyncpg.Pool) -> User:
